...
simxInt *left_sensor_handle = new simxInt[1]();
simxInt *middle_sensor_handle = new simxInt[1]();
simxInt *right_sensor_handle = new simxInt[1]();
...
simxUChar left_line_sensor_data = 0;
simxUChar middle_line_sensor_data = 0;
simxUChar right_line_sensor_data = 0;
...
void get_object_handles(void)
{
	...
	return_code = simxGetObjectHandle(client_id, "LineTracer", eBot_handle, simx_opmode_blocking);

	if (return_code == simx_return_ok)
	{
		simxGetObjectHandle(client_id, "DynamicLeftJoint", left_joint_handle, simx_opmode_blocking);
		simxGetObjectHandle(client_id, "DynamicRightJoint", right_joint_handle, simx_opmode_blocking);
		simxGetObjectHandle(client_id, "LeftSensor", left_sensor_handle, simx_opmode_blocking);
		simxGetObjectHandle(client_id, "MiddleSensor", middle_sensor_handle, simx_opmode_blocking);
		simxGetObjectHandle(client_id, "RightSensor", right_sensor_handle, simx_opmode_blocking);
...
{
	return_code = -1;
	// return_code = simxGetVisionSensorImage(client_id, *left_sensor_handle, resolution, &left_line_sensor_data, 1, simx_opmode_buffer);
	return_code = simxReadVisionSensor(client_id, *left_sensor_handle, &left_line_sensor_data, NULL, NULL, simx_opmode_buffer);
	// return_code = simxGetVisionSensorImage(client_id, *middle_sensor_handle, resolution, &middle_line_sensor_data, 1, simx_opmode_buffer);
	return_code = simxReadVisionSensor(client_id, *middle_sensor_handle, &middle_line_sensor_data, NULL, NULL, simx_opmode_buffer);
	// return_code = simxGetVisionSensorImage(client_id, *right_sensor_handle, resolution, &right_line_sensor_data, 1, simx_opmode_buffer);
	return_code = simxReadVisionSensor(client_id, *right_sensor_handle, &right_line_sensor_data, NULL, NULL, simx_opmode_buffer);
}
...
void init_vision_sensors(void)
{
	...
	do
	{
		// return_code = simxGetVisionSensorImage(client_id, *left_sensor_handle, resolution, &left_line_sensor_data, 1, simx_opmode_streaming);
		// get_white_line_sensor_data();
		return_code = simxReadVisionSensor(client_id, *left_sensor_handle, &left_line_sensor_data, NULL, NULL, simx_opmode_streaming);
	} while (return_code != simx_return_ok || return_code == simx_return_novalue_flag);

	do
	{
		// return_code = simxGetVisionSensorImage(client_id, *middle_sensor_handle, resolution, &middle_line_sensor_data, 1, simx_opmode_streaming);
		// get_white_line_sensor_data();
		return_code = simxReadVisionSensor(client_id, *middle_sensor_handle, &middle_line_sensor_data, NULL, NULL, simx_opmode_streaming);
	} while (return_code != simx_return_ok || return_code == simx_return_novalue_flag);

	do
	{
		// return_code = simxGetVisionSensorImage(client_id, *right_sensor_handle, resolution, &right_line_sensor_data, 1, simx_opmode_streaming);
		// get_white_line_sensor_data();
		return_code = simxReadVisionSensor(client_id, *right_sensor_handle, &right_line_sensor_data, NULL, NULL, simx_opmode_streaming);
	} while (return_code != simx_return_ok || return_code == simx_return_novalue_flag);
